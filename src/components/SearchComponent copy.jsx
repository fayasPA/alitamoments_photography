...
const SearchComponent = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-white md:px-6 py-10 pt-32">
      {/* Search Bar */}
      <input
        type="text"
      ...
      <div className="flex flex-wrap -mx-2">
        {filteredCollections.map((collection, index) => (
          <div
            key={index}
            className="w-1/2 lg:w-1/4 xl:w-1/6 px-3 md:px-10 mb-4 collection-item"
          >
            {/* Image Container */}
            <div className="relative w-full h-0 overflow-hidden" style={{ paddingBottom: '150%' }}>
              <img
                ...
                alt={collection.name}
                className="absolute inset-0 w-full h-full object-cover shadow-lg hover:scale-125 duration-[2000ms] transition-all"
              />
            </div>
